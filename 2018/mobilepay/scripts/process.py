...
Entry = namedtuple('Entry', ['kind', 'amount', 'dt', 'msg', 'fee', 'balance'])
...
def read_entry(row):
    ...
    return Entry(kind, amount, dt, msg, fee, balance)
...
def writecsv(path, fieldnames, entries):
...
def main():
    ...
    outpath = basename(entries_path) + "-" + basename(ruleset_path) + ".csv"
    ...
    writecsv(outpath, fieldnames, entries)
